;(function($) {
	...
	$('#chili-satosms_message_diff_status\\[enable_diff_status_mesg\\]').on( 'change', function() {
		var self = $(this),
			value = self.val();
		if ( self.is(':checked')) {
			$('.satosms_hide_class').hide();
			$('.satosms_different_message_status_wrapper').fadeIn();
		} else {
			$('.satosms_hide_class').hide();
		}
	});
	...
	$('#chili-satosms_message_diff_status\\[enable_diff_status_mesg\\]').trigger('change');
